...
export const ZStrykerRunInputShape: z.ZodRawShape = {
	...
	mutate: z
		.array(z.string())
		.optional()
		.describe('Glob(s) of files to mutate (comma-joined for CLI)'),
	...
	buildCommand: z
		.string()
		.optional()
		.describe("Command to run before tests (e.g. 'npm run build')"),
	coverageAnalysis: z.enum(['off', 'all', 'perTest']).optional(),
	testRunner: z
		.string()
		.optional()
		.describe('jest | mocha | jasmine | karma | vitest | tap | cucumber | command'),
	testRunnerNodeArgs: z
		.array(z.string())
		.optional()
		.describe('Node exec args for test runner child process'),
	checkers: z.array(z.string()).optional().describe("Checker plugins e.g. ['typescript']"),
	checkerNodeArgs: z
		.array(z.string())
		.optional()
		.describe('Node exec args for checker child process'),
	...
	configFile: z
		.string()
		.optional()
		.describe('Path to an existing Stryker config file (passed as the last arg)'),
	// TODO: Fix the configOverrides typing for Zod 4.0, if we start using this schema again for an MCP Stryker config tool
	configOverrides: z
		.record(z.string(), z.any())
		.optional()
		.describe(
			'Partial Stryker config to write to a temp JSON file and use as the config for this run',
		),
