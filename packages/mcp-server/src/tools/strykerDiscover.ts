...
async function strykerDiscoverHandler(
    ...
    try {        
        ...
        if (!strykerServer.isInitialized()) {
            return {
                content: [{ type: "text", text: "Stryker server has not started. Please call `strykerStart` first." }],
                isError: true,
            };
        }        
        if (args.files) {
            console.error(`[strykerDiscover] Processing ${args.files.length} file path(s)/pattern(s)`);
        ...
        console.error(`[strykerDiscover] Calling strykerServer.discover with params: ${JSON.stringify(args)}`);
