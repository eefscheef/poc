...
describe('LLM Connection Tests', () => {
  ...
  test('environment should have model configured', () => {
    ...
    if (hasOpenAIKey) {
      assert.ok(
        ...
        'OPENAI_MODEL must be set when using OpenAI'
      );
    }
    ...
    if (hasGeminiKey && !hasOpenAIKey && !hasAnthropicKey) {
      assert.ok(
        ...
        'GOOGLE_MODEL must be set when using Gemini'
      );
    }
  });
  ...
  test('should resolve LLM config for Anthropic', { skip: !process.env.ANTHROPIC_API_KEY }, () => {
  ...
  test('should connect to LLM and get response', { 
    skip: !process.env.RUN_LIVE_TESTS || (!process.env.OPENAI_API_KEY && !process.env.ANTHROPIC_API_KEY && !process.env.GOOGLE_API_KEY)
  }, async () => {
    const config = resolveLLMConfig();
    const llm = createLLM(config);
